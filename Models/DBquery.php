...
class DBquery
{
    ...
    public static function whereOrm($column, $operator, $value, $isRaw = false)
    {
        if ($isRaw) {
...
//        self::$offset = 0;
     ...
     */
    public static function get($columnArr = [], array $where = [])
    {
        $db = self::connectDB();
        $table = static::$table;
    ...
    public static function get2($columnArr = [], array $where = [], $FETCH_ALL = 2)
    {
        $db = self::connectDB();
        $table = static::$table;
